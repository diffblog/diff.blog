...
def companies_by_topic(request):
    ...
    user_ids = Post.objects.filter(topics__slug=topic_slug).values_list("profile", flat=True).distinct()
    users = UserProfile.objects.filter(id__in=list(user_ids), is_organization=True).order_by('-followers_count')
