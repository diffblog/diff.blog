...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for raw_topic in raw_topics:
            ...
            domain_topics = Topic.objects.filter(category=domain_category, display_name=raw_topic.display_name)
            present = False
            ...
            domain_topics = Topic.objects.filter(category=language_category, display_name=raw_topic.display_name)
            if domain_topics:
                domain_topic = domain_topics[0]
                posts = raw_topic.posts.all()
                for post in posts:
